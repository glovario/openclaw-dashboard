    ...
    >
      <div className="card-body py-3">
        <div className="d-flex align-items-start justify-content-between gap-2">
          <div className="flex-grow-1 task-card-body">
            <div className="d-flex align-items-center gap-2 mb-1 task-card-body">
              {task.display_id && (
                <span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color: '#6c757d', letterSpacing: '0.02em', userSelect: 'none', flexShrink: 0 }}>
                  {task.display_id}
                </span>
              )}
              <h6 className="card-title mb-0 task-card-title">{task.title}</h6>
            </div>
            {task.description && (
              <p className="card-text text-muted small mb-2 task-card-description">{task.description}</p>
            )}
